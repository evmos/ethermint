...
function checkTestEnv() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .boolean('verbose-log').describe('verbose-log', 'print entangled output, default false')
    .argv;
...
function setupNetwork({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    const entangledProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', runConfig.verboseLog ? 'pipe' : 'ignore', 'pipe'],
    });

    logger.info(`Starting entangled process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      entangledProc.stdout.pipe(process.stdout);
    }
    entangledProc.stderr.on('data', d => {
      const oLine = d.toString();
      if (runConfig.verboseLog) {
        process.stdout.write(oLine);
      }
      if (oLine.indexOf('Starting JSON-RPC server') !== -1) {
        logger.info('entangled started');
        resolve(entangledProc);
      }
    });
  });

  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start entangled timeout!')), timeout);
  });

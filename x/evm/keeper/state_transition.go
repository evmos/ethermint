...
func (k *Keeper) ApplyTransaction(tx *ethtypes.Transaction) (*types.MsgEthereumTxResponse, error) {
	...
	result, err := k.ApplyMessage(evm, msg, ethCfg)
	if err != nil {
		return nil, err
	}
	...
	receipt := &ethtypes.Receipt{
		Type:              tx.Type(),
		Status:            ethtypes.ReceiptStatusSuccessful, // TODO: figure out failed receipts
		CumulativeGasUsed: k.ctx.BlockGasMeter().GasConsumed(),
		TxHash:            txHash,
		GasUsed:           result.GasInfo.GasConsumed,
		Logs:              k.GetLogs(txHash),
		ContractAddress:   common.HexToAddress(result.Response.ContractAddress),
		BlockHash:         k.headerHash,
		BlockNumber:       big.NewInt(k.ctx.BlockHeight()),
...
func (k *Keeper) ApplyMessage(evm *vm.EVM, msg core.Message, cfg *params.ChainConfig) (*types.ExecutionResult, error) {
	var (

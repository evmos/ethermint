...
func (k Keeper) TraceTx(c context.Context, req *types.QueryTraceTxRequest) (*types.QueryTraceTxResponse, error) {
	...
	for i, tx := range req.Predecessors {
		...
		if _, err := k.ApplyMessageWithConfig(ctx, msg, types.NewNoOpTracer(), true, cfg); err != nil {
			continue
		}
	}
...
func (k *Keeper) traceTx(
	...
	case traceConfig != nil && traceConfig.Tracer != "":
		...
		tCtx := &tracers.Context{
			BlockHash: k.GetHashFn(ctx)(uint64(ctx.BlockHeight())),
			TxIndex:   int(txIndex),
			TxHash:    txHash,
		}
	...
	res, err := k.ApplyMessage(ctx, msg, tracer, commitMessage)
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}

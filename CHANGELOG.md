...
* (rpc) [#377](https://github.com/tharsis/ethermint/pull/377) Implement miner namespace, `SetEtherbase` and `SetGasPrice` are working as intended. All the other calls are not relevant to `ethermint`, these calls use the same interface as `go-eth` but return `unsupported`.

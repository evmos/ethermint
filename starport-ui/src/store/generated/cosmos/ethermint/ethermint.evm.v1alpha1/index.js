    ...
    actions: {
        ...
        async QueryTxReceipt({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryTxReceipt', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryTxReceiptsByBlockHeight({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryTxReceiptsByBlockHeight', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryTxReceiptsByBlockHash({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryTxReceiptsByBlockHash', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryBlockBloom({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryBlockBloom({ ...query, 'pagination.key': value.pagination.nextKey })).data;
        ...
        async QueryStaticCall({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryStaticCall({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    for (let prop of Object.keys(next_values)) {
                        if (Array.isArray(next_values[prop])) {
                            value[prop] = [...value[prop], ...next_values[prop]];
                        }
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryStaticCall', payload: { options: { all }, params: { ...key }, query } });

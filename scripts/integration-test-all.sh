...
init_func() {
    "$PWD"/build/entangled keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
    "$PWD"/build/entangled init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
    "$PWD"/build/entangled add-genesis-account \
    "$("$PWD"/build/entangled keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000aENTGL,1000000000000000000stake \
    --keyring-backend test --home "$DATA_DIR$i"
    "$PWD"/build/entangled gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"

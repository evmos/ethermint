...
func TestClientRequest(t *testing.T) {
	...
	defer server.Stop()
	client := rpcServer.DialInProc(server)
	defer client.Close()
...
func TestClientErrorData(t *testing.T) {
	...
	// Check data.
	if e, ok := err.(types.DataError); !ok {
...
func TestClientBatchRequest(t *testing.T) {
	...
	wantResult := []rpcClient.BatchElem{
		{
			Method: "test_echo",
			Args:   []interface{}{"hello", 10, &echoArgs{"world"}},
...
func (r *unsubscribeRecorder) ReadBatch() ([]*codec.JsonrpcMessage, bool, error) {
...
func TestClientSubscriptionUnsubscribeServer(t *testing.T) {
	...
	p1, p2 := net.Pipe()
	recorder := &unsubscribeRecorder{ServerCodec: codec.NewCodec(p1)}
	go srv.ServeCodec(recorder, rpcServer.OptionMethodInvocation|rpcServer.OptionSubscriptions)
	defer srv.Stop()

	// Create the client on the other end of the pipe.
	client, _ := rpcClient.NewClient(context.Background(), func(context.Context) (codec.ServerCodec, error) {
...
func TestClientSubscriptionChannelClose(t *testing.T) {
	...
	srv.RegisterName("nftest", new(notificationTestService))
	client, _ := rpcClient.Dial(wsURL)

	for i := 0; i < 100; i++ {
		ch := make(chan int, 100)
		sub, err := client.Subscribe(context.Background(), "nftest", ch, "someSubscription", types.MaxClientSubscriptionBuffer-1, 1)
		if err != nil {
			t.Fatal(err)
		}
		sub.Unsubscribe()
...
func TestClientNotificationStorm(t *testing.T) {
	...
	doTest := func(count int, wantError bool) {
		...
		for i := 0; i < count; i++ {
			...
			case err := <-sub.Err():
				if wantError && err != rpcClient.ErrSubscriptionQueueOverflow {
					t.Fatalf("(%d/%d) got error %q, want %q", i, count, err, rpcClient.ErrSubscriptionQueueOverflow)
				} else if !wantError {
...
func TestClientSetHeader(t *testing.T) {
	...
	client, err := rpcClient.Dial(httpsrv.URL)
	if err != nil {
		t.Fatal(err)
	}
	defer client.Close()
...
func TestClientReconnect(t *testing.T) {
	startServer := func(addr string) (*rpcServer.Server, net.Listener) {
		srv := newTestServer()
	...
	client, err := rpcClient.DialContext(ctx, "ws://"+l1.Addr().String())
	if err != nil {
...
func httpTestClient(srv *rpcServer.Server, transport string, fl *flakeyListener) (*rpcClient.Client, *httptest.Server) {
	...
	// Connect the client.
	hs.Start()
	client, err := rpcClient.Dial(transport + "://" + hs.Listener.Addr().String())
	if err != nil {
		panic(err)
	}
	return client, hs
...
func ipcTestClient(srv *rpcServer.Server, fl *flakeyListener) (*rpcClient.Client, net.Listener) {
	...
	// Connect the client.
	client, err := rpcClient.Dial(endpoint)
	if err != nil {
		panic(err)
	}
	return client, l

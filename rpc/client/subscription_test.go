...
func TestSubscriptions(t *testing.T) {
	var (
		...
		notifications          = make(chan codec.SubscriptionResult)
	...
	go server.ServeCodec(codec.NewCodec(serverConn), 0)
	defer server.Stop()
...
func TestServerUnsubscribe(t *testing.T) {
	...
	go server.ServeCodec(codec.NewCodec(p1), 0)
...
func waitForMessages(in *json.Decoder, successes chan subConfirmation, notifications chan codec.SubscriptionResult, errors chan error) {
	for {
...
func readAndValidateMessage(in *json.Decoder) (*subConfirmation, *codec.SubscriptionResult, error) {
	var msg codec.JsonrpcMessage
	if err := in.Decode(&msg); err != nil {
		return nil, nil, fmt.Errorf("decode error: %v", err)
	}
	switch {
	case msg.IsNotification():
		var res codec.SubscriptionResult

...
func sequentialIDGenerator() func() types.ID {
	var (
		mu      sync.Mutex
		counter uint64
	)
...
func (s *testService) PeerInfo(ctx context.Context) codec.PeerInfo {
	return codec.PeerInfoFromContext(ctx)
}
...
func (s *testService) Subscription(ctx context.Context) (*types.Subscription, error) {
	return nil, nil
}
...
func (s *notificationTestService) SomeSubscription(ctx context.Context, n, val int) (*types.Subscription, error) {
	notifier, supported := client.NotifierFromContext(ctx)
	if !supported {
...
func (s *notificationTestService) HangSubscription(ctx context.Context, val int) (*types.Subscription, error) {
	notifier, supported := client.NotifierFromContext(ctx)
	if !supported {
		return nil, types.ErrNotificationsUnsupported
	}

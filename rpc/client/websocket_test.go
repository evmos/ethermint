...
import (
	...
	"github.com/evmos/ethermint/rpc/server"
...
func TestWebsocketClientHeaders(t *testing.T) {
	...
	endpoint, header, err := rpcClient.WsClientHeaders("wss://testuser:test-PASS_01@example.com:1234", "https://example.com")
	if err != nil {
...
func TestWebsocketOriginCheck(t *testing.T) {
	...
	client, err := rpcClient.DialWebsocket(context.Background(), wsURL, "http://ekzample.com")
	if err == nil {
		client.Close()
		t.Fatal("no error for wrong origin")
	}
	wantErr := types.WsHandshakeError{Err: websocket.ErrBadHandshake, Status: "403 Forbidden"}
	if !errors.Is(err, wantErr) {
		t.Fatalf("wrong error for wrong origin: %q", err)
	}

	// Connections without origin header should work.
	client, err = rpcClient.DialWebsocket(context.Background(), wsURL, "")
	if err != nil {
...
func TestWebsocketLargeCall(t *testing.T) {
	...
	arg := strings.Repeat("x", types.MaxRequestContentLength-200)
	...
	arg = strings.Repeat("x", types.MaxRequestContentLength*2)
...
func TestWebsocketPeerInfo(t *testing.T) {
	...
	c, err := rpcClient.DialWebsocket(ctx, tsurl, "origin.example.com")
	if err != nil {
		t.Fatal(err)
	}
...
func TestClientWebsocketPing(t *testing.T) {
	...
	client, err := rpcClient.DialContext(ctx, "ws://"+server.Addr)
	if err != nil {
...
func TestClientWebsocketLargeMessage(t *testing.T) {
	...
	respLength := types.WsMessageSizeLimit - 50
	srv.RegisterName("test", largeRespService{respLength})

	c, err := rpcClient.DialWebsocket(context.Background(), wsURL, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestClientWebsocketSevered(t *testing.T) {
	...
	client, err := rpcClient.DialWebsocket(ctx, wsURL, "")
	if err != nil {
		t.Fatalf("client dial error: %v", err)
	}
	defer client.Close()
	...
	timeout := time.NewTimer(3 * types.WsPingInterval)
	defer timeout.Stop()
	for {
		select {
		case err := <-sub.Err():

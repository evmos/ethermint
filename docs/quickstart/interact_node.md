...
```bash
ethermintcli query bank balances $MY_VALIDATOR_ADDRESS --chain-id=ethermint_9000-1
```
...
```bash
ethermintcli keys add recipient --keyring-backend=file

# Put the generated address in a variable for later use.
RECIPIENT=$(ethermintcli keys show recipient -a --keyring-backend=file)
```

The command above creates a local key-pair that is not yet registered on the chain. An account is created the first time it receives tokens from another account. Now, run the following command to send tokens to the `recipient` account:

```bash
ethermintcli tx bank send $MY_VALIDATOR_ADDRESS $RECIPIENT 1000000aphoton --chain-id=ethermint_9000-1 --keyring-backend=file

# Check that the recipient account did receive the tokens.
ethermintcli query bank balances $RECIPIENT --chain-id=ethermint_9000-1
```

Finally, delegate some of the stake tokens sent to the `recipient` account to the validator:

```bash
ethermintcli tx staking delegate $(ethermintcli keys show my_validator --bech val -a --keyring-backend=file) 500aphoton --from=recipient --chain-id=ethermint_9000-1 --keyring-backend=file

# Query the total delegations to `validator`.
ethermintcli query staking delegations-to $(ethermintcli keys show my_validator --bech val -a --keyring-backend=file) --chain-id=ethermint_9000-1
```

...
func TestSDKTxFeeChecker(t *testing.T) {
	...
	testCases := []struct {
		...
		{"success, genesis tx", genesisCtx, MockEVMKeeper{}, func() sdk.Tx {
			return encodingConfig.TxConfig.NewTxBuilder().GetTx()
		}, "", 0, true},
		{"fail, min-gas-prices", checkTxCtx, MockEVMKeeper{}, func() sdk.Tx {
			return encodingConfig.TxConfig.NewTxBuilder().GetTx()
		}, "", 0, false},
		{"success, min-gas-prices", checkTxCtx, MockEVMKeeper{}, func() sdk.Tx {
			txBuilder := encodingConfig.TxConfig.NewTxBuilder()
			txBuilder.SetGasLimit(1)
			txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewCoin("aphoton", sdk.NewInt(10))))
			return txBuilder.GetTx()
		}, "10aphoton", 0, true},
		{"success, min-gas-prices deliverTx", deliverTxCtx, MockEVMKeeper{}, func() sdk.Tx {
			return encodingConfig.TxConfig.NewTxBuilder().GetTx()
		}, "", 0, true},
		{"fail, dynamic fee", deliverTxCtx, MockEVMKeeper{
		...
		{"success, dynamic fee", deliverTxCtx, MockEVMKeeper{
		...
		{"success, dynamic fee priority", deliverTxCtx, MockEVMKeeper{
			EnableLondonHF: true, BaseFee: big.NewInt(10),
		}, func() sdk.Tx {
			txBuilder := encodingConfig.TxConfig.NewTxBuilder()
			txBuilder.SetGasLimit(1)
			...
			return txBuilder.GetTx()
		}, "10000010aphoton", 10, true},
		{"success, dynamic fee empty tipFeeCap", deliverTxCtx, MockEVMKeeper{
			EnableLondonHF: true, BaseFee: big.NewInt(10),
		}, func() sdk.Tx {
		...
		{"success, dynamic fee tipFeeCap", deliverTxCtx, MockEVMKeeper{
			EnableLondonHF: true, BaseFee: big.NewInt(10),
		}, func() sdk.Tx {
			txBuilder := encodingConfig.TxConfig.NewTxBuilder().(authtx.ExtensionOptionsTxBuilder)
			txBuilder.SetGasLimit(1)
			...
			txBuilder.SetExtensionOptions(option)
			return txBuilder.GetTx()
		}, "5000010aphoton", 5, true},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			fees, priority, err := NewSDKTxFeeChecker(tc.keeper)(tc.ctx, tc.buildTx())
			if tc.expSuccess {
